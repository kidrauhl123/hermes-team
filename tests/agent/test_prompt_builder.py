...
class TestBuildContextFilesPrompt:
    ...
    def test_loads_soul_md_from_hermes_home_only(self, tmp_path, monkeypatch):
        ...
        hermes_home.mkdir()
        (hermes_home / "SOUL_base.md").write_text("Be concise and friendly.", encoding="utf-8")
        (tmp_path / "SOUL_base.md").write_text("cwd soul should be ignored", encoding="utf-8")
        result = build_context_files_prompt(cwd=str(tmp_path))
    ...
    def test_soul_md_has_no_wrapper_text(self, tmp_path, monkeypatch):
        ...
        assert "## SOUL_base.md" not in result
    ...
    def test_empty_soul_md_adds_nothing(self, tmp_path, monkeypatch):
        ...
        hermes_home = tmp_path / "hermes_home"
        hermes_home.mkdir()
        (hermes_home / "SOUL_base.md").write_text("\n\n", encoding="utf-8")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert result == ""
